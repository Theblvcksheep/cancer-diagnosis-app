...
def add_predict(input_data: dict):
    model: LogisticRegression = pkl.load(open(r"..\model\model.pkl", 'rb'))
    scaler: StandardScaler = pkl.load(open(r"..\model\scaler.pkl", 'rb'))

    input_arr = np.array(list(input_data.values())).reshape(1,-1)
    input_arr_scaled = scaler.transform(input_arr)

    predict = model.predict(input_arr_scaled)
